...
def get_salary_table_for_job_title(job_title: str) -> pd.DataFrame:
    dprint('...in get_salary_table_for_job_title')
    ...
    if(data_io.file_exists(job_cache_name)):
        dprint('salary already in cache')
    ...
    # else, scrape the job, save it in the cache, then return
        ...
        dprint('salary not in cache, scraped. salary_df after replacement:')
        dprint(salary_df.head())
